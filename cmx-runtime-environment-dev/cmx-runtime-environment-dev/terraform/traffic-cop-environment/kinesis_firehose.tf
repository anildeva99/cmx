resource "aws_kinesis_firehose_delivery_stream" "cloudwatch_to_es" {
  ...
  name        = "CMXTrafficCop-${var.environment}-${random_string.upper_case_string[each.key].result}-cloudwatch_to_es"
  ...
  elasticsearch_configuration {
    ...
    vpc_config {
      ...
      security_group_ids = ["${aws_security_group.firehose_stream_sg.id}"]
      role_arn           = aws_iam_role.firehose_to_elastic_search_role.arn
    }
  ...
  tags = merge(
    var.shared_resource_tags,
    {
      ...
      Name = "CodaMetrix Traffic Cop Kinesis Delivery Stream - cloudwatch_to_es"
    }
  )
}
