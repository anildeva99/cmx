resource "aws_kinesis_stream" "kinesis_stream_to_push_cloudwatch_log_group_to_es" {
  ...
  name     = "CMXTrafficCop-${var.environment}-${random_string.upper_case_string[each.key].result}-kinesis_stream_to_${each.value.log_template_name}"
  ...
  tags = merge(
    var.shared_resource_tags,
    {
      ...
      Name = "CodaMetrix Traffic Cop Kinesis - kinesis_stream_to_push_cloudwatch_log_group_to_es"
    }
  )
}
