...
resource "aws_lambda_function" "cloudwatch_log_groups_to_es" {
  ...
  function_name    = "CMXTrafficCop-${var.environment}-cloudwatch_log_groups_to_es"
  ...
  tags = merge(
    var.shared_resource_tags,
    {
      ...
      Name = "CodaMetrix Traffic Cop Lambda - cloudwatch_log_groups_to_es"
    }
  )
}
...
resource "aws_lambda_function" "elasticsearch_rotate_indices" {
  ...
  function_name = "CMXTrafficCop-${var.environment}-${each.key}-elasticsearch_rotate_indices"
  ...
  tags = merge(
    var.shared_resource_tags,
    {
      ...
      Name = "CodaMetrix Traffic Cop Lambda - ${var.environment}-${each.key}-elasticsearch_rotate_indices"
    }
  )
}
