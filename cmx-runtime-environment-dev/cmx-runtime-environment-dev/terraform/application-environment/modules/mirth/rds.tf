resource "aws_db_instance" "mirth_database" {
  ...
  auto_minor_version_upgrade      = var.mirth_database_multi_az ? true : false
  ...
  tags = merge(
    ...
    {
      Type = "mirth_database"
      Name = "CodaMetrix Application - mirth_database"
    }
  )
}
...
resource "aws_db_subnet_group" "mirth_database_subnet_group" {
  ...
  tags = merge(
    var.shared_resource_tags,
    {
      Name             = "CodaMetrix Application - mirth_database_subnet_group"
